...
void input_vgmstream::load_settings() noexcept
{
    ...
    (void)sscanf(cfg_LoopCount.get_ptr(), "%lf", &loop_count);
    (void)sscanf(cfg_FadeDelay.get_ptr(), "%lf", &fade_delay_seconds);

    loop_forever = cfg_LoopForever;
    ignore_loop = cfg_IgnoreLoop;
    disable_subsongs = cfg_DisableSubsongs;

    (void)sscanf(cfg_DownmixChannels.get_ptr(), "%d", &downmix_channels);
...
{
    if ((accept_unknown == nullptr) || (accept_common == nullptr))
        return;
...
const char * vgmstream_prefs::get_name() noexcept
{
    return input_vgmstream::g_get_name();
}

GUID vgmstream_prefs::get_guid() noexcept
{
    return input_vgmstream::g_get_preferences_guid();
}

GUID vgmstream_prefs::get_parent_guid() noexcept
{
    return guid_input;
}

t_uint32 vgmstreamPreferences::get_state() noexcept
{
...
void vgmstreamPreferences::reset() noexcept
{
...
void vgmstreamPreferences::apply()
{
    ...
    if (sscanf(buf.get_ptr(), "%lf%n", &temp_fade_seconds, &consumed) < 1 || consumed != strlen(buf.get_ptr()) || temp_fade_seconds < 0)
    {
        uMessageBox(m_hWnd,
        ...
        cfg_FadeLength = buf.get_ptr();

    buf = uGetDlgItemText(m_hWnd, IDC_LOOP_COUNT);

    if (sscanf(buf.get_ptr(), "%lf%n", &temp_loop_count, &consumed) < 1 || consumed != strlen(buf.get_ptr()) || temp_loop_count < 0)
    {
        uMessageBox(m_hWnd,
        ...
        cfg_LoopCount = buf.get_ptr();

    buf = uGetDlgItemText(m_hWnd, IDC_FADE_DELAY_SECONDS);

    if (sscanf(buf.get_ptr(), "%lf%n", &temp_fade_delay_seconds, &consumed) < 1 || consumed != strlen(buf.get_ptr()) || temp_fade_delay_seconds < 0)
    {
        uMessageBox(m_hWnd,
        ...
        cfg_FadeDelay = buf.get_ptr();

    buf = uGetDlgItemText(m_hWnd, IDC_DOWNMIX_CHANNELS);

    if (sscanf(buf.get_ptr(), "%d%n", &temp_downmix_channels, &consumed) < 1 || consumed != strlen(buf.get_ptr()) || temp_downmix_channels < 0)
    {
        uMessageBox(m_hWnd,
        ...
        cfg_DownmixChannels = buf.get_ptr();
}
...
static preferences_page_factory_t<vgmstream_prefs> _PreferencesPageFactory;
